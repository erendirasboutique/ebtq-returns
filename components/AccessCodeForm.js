...
export default function AccessCodeForm({ onVerified }) {
  ...
  return (
    <section className="card accessCard">
      ...
      <p className="muted">Use the access code provided by Erendira&apos;s Boutique to start your return request.</p>

      {msg && <div className={`notice ${msg.toLowerCase().includes('not') || msg.toLowerCase().includes('invalid') ? 'error' : ''}`}>{msg}</div>}
      ...
      <form onSubmit={verifyCode} className="form">
        <label>
          ...
          <input
            ...
            placeholder="Example: EBTQ-ROSE-4821"
            required
          />
        </label>
